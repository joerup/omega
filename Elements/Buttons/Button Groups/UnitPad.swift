                ...
                LazyVGrid(columns: Array(repeating: GridItem(.flexible(), spacing: 0), count: count), spacing: 0) {
                    ...
                    ForEach(0..<(count*Int(geometry.size.height/buttonHeight)), id: \.self) { index in
                        ...
                        if index < buttons.count {
                            ButtonView(button: buttons[index], input: queue, backgroundColor: color(buttons[index].name == "Ω" ? (theme ?? self.settings.theme).color1 : (theme ?? self.settings.theme).color3), width: width*0.95/CGFloat(count), height: buttonHeight, relativeSize: 0.45, active: active, showText: showText, onChange: onChange)
